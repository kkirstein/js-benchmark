...
// This is a CLI entry point for node.js
//
...
(async () => {
    ...
    const fib_naive = await t.time_it_async(fib.fib_naive, 35);
    console.log("fib_naive(35) = ", fib_naive.result, "\tElapsed: ", fib_naive.elapsed, "ms.");

    const fib_35 = await t.time_it_async(fib.fib, 35);
    console.log("fib(35) = ", BigInt.bigInt2str(fib_35.result, 10),
        "\tElapsed: ", fib_35.elapsed, "ms.");

    const fib_1000 = await t.time_it_async(fib.fib, 1000);
    ...
    let tic12 = new Date();
    let promise = mandel.mandelbrot_async(640, 480, -0.5, 0.0, 4.0/640);
